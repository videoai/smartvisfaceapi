...
namespace SmartVisFaceAPI
{
    ...
    class Program
    {
        ...
        private static MySession Login()
        {
            ...
            string base_string = method + "&" + WebUtility.UrlEncode(auth_endpoint) + "&" + WebUtility.UrlEncode(parameters);
        ...
        {
            var device_data = "device_id=\"id1234\", device_name=\"mydevice\", lat=\"0\", lng=\"0\"";
            var oauth_nonce = GetNonce();
            var oauth_timestamp = GetTimeStamp();
            ...
            var base_string = method + "&" + WebUtility.UrlEncode(subject_end_point) + "&" + WebUtility.UrlEncode(parameters);
